...
{
	sc_MasterServer *master = (sc_MasterServer*)calloc(sizeof(sc_MasterServer), 1);
...
void SC_API(sc_getServers)(sc_MasterServer *master, sc_Region region, const char *filter)
{
	...
	char *message = (char*)calloc(sizeof(char), 2 + ADDRSTRLEN + filterLen);	// data to send
	...
	do {
		...
		if (sent = send(master->socket, message, 2 + addressLen + filterLen, 0) < (signed)(2 + addressLen + filterLen)) {
			SC_ERRORMSG("Unable to send all data", SC_SEND_ERROR);
			break;
		}
		
		if ((recvd = recv(master->socket, (char*)&buffer, 1392, 0)) < 0) {
		...
		for (i = 0; i < recvd; i += 6) {
			...
			if (strcmp(address, "0.0.0.0:0")) {
				sc_ServerList *list = (sc_ServerList*)malloc(sizeof(sc_ServerList));
				strcpy(list->address, addr2);
				sprintf(&list->port[0], "%d", port);
				list->next = NULL;
				if (last) {
					last->next = list;
				} else {
